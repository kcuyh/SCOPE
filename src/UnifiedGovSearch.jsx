...
function CategoryCard({ icon, title, desc, onClick }) {
  return (
    <button className="cat" onClick={onClick}>
      ...
      <style jsx>{`
        .cat {
          width: clamp(220px, 28vw, 280px); /* 가운데 정렬 안정화를 위한 폭 */
          display: flex;
          flex-direction: column;
          gap: 6px;
          padding: 16px 18px;
    ...
    <span>
      ...
      {isTruncated && (
        <button
          onClick={() => setExpanded(!expanded)}
          style={{
            marginLeft: 6, color: "#0ea5e9", cursor: "pointer",
            background: "none", border: "none", fontSize: "0.9em"
          }}
        >
          {expanded ? "접기" : "더보기"}
        </button>
...
function ServiceCard({ it }) {
  return (
    <div style={{
      ...
      <div className="svc-detail-grid">
        {it.supportTarget && (
          <Row icon="👥" label={<><b>대상:</b> <ExpandableText text={it.supportTarget} /></>} />
        )}
        {it.selectionCriteria && (
          <Row icon="📝" label={<><b>선정요건:</b> <ExpandableText text={it.selectionCriteria} /></>} />
        )}
        {it.applicationDeadline && (
          <Row icon="📅" label={<><b>신청기간:</b> <ExpandableText text={it.applicationDeadline} /></>} />
        )}
        {it.institutionName && (
          <Row icon="🏢" label={<><b>기관:</b> <ExpandableText text={it.institutionName} /></>} />
        )}
      </div>
  ...
  return (
    <div className="card event-grid">
      <div className="event-main">
        ...
        <div className="sub">📍 장소 <ExpandableText text={it.eventPlc || it.location || "-"} /></div>
        <div className="sub">🗓 일시 <ExpandableText text={it.eventTermDetail || it.date || "-"} /></div>
        <div className="sub">🏢 주최 <ExpandableText text={it.eventPlc || it.org || "-"} /></div>
...
function CompanyCard({ it }) {
  return (
    <div className="card">
      ...
      {it.coIntroCont && (<div className="sub">📍 기업설명: <ExpandableText text={it.coIntroCont} /></div>)}
      {it.coClcdNm    && (<div className="sub">🗓 <ExpandableText text={it.coClcdNm} /></div>)}
      {it.mainBusiCont&& (<div className="sub">🏢 <ExpandableText text={it.mainBusiCont} /></div>)}
      {it.coIntroSummaryCont && (<div style={{marginTop:10}}><ExpandableText text={it.coIntroSummaryCont} /></div>)}
...
export default function UnifiedGovSearch() {
  ...
  const renderServiceList = (payload) => {
    ...
    if (items.length === 0) return <div className="empty">검색 결과가 없습니다. 다른 키워드로 다시 시도해 보세요.</div>;
    return <div className="card-list">{items.map((it,i)=><ServiceCard key={i} it={it} />)}</div>;
  };
  const renderEventList = (payload) => {
    const items = toItems(payload);
      ...
      <div className="wrap">
        ...
        <div style={{ display: "flex", justifyContent: "center" }}>
          ...
          >
            <div
              style={{
                width: 80, height: 80, borderRadius: 16, background: "#e0f2fe",
                display: "grid", placeItems: "center", color: "#0284c7",
                boxShadow: "0 10px 30px rgba(2,132,199,.12)", fontSize: 60,
              }}
            >
              <svg width="60" height="60" viewBox="0 0 24 24"
            ...
            <div>
              <div style={{ fontWeight: 900, letterSpacing: 0.5, fontSize: "clamp(24px, 4vw, 40px)" }}>
                SCOPE
              </div>
              <div className="sub" style={{ fontSize: "clamp(12px, 1.6vw, 16px)" }}>
                청년정보 어시스턴트
